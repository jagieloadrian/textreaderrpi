...
/** Point-in-time snapshot of a [ResourceTracker]'s state. */
@Serializable
...
class ResourceTracker(
    ...
    /** Returns a consistent snapshot of capacity, active slots, available slots, and closed state. */
    val snapshot: ResourceSnapshot
        get() = ResourceSnapshot(
            capacity = maxSlots,
            active = heldCount,
            available = maxSlots - heldCount,
            isClosed = isClosed,
        )

    init {
        metricRegistry?.apply {
            register("resource.tracker.$trackerName.active", Gauge { heldCount })
            register("resource.tracker.$trackerName.available", Gauge { maxSlots - heldCount })
        }
    }
