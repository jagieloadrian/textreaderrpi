...
class LcdDisplayTest : FunSpec({
    ...
    test("should represent I2C initialization failure in DisplayStatus") {
        val status = DisplayStatus(
            isActive = false, hardwareAvailable = false,
            currentMessage = null, error = "I2C initialization failed"
        )
        status.hardwareAvailable shouldBe false
        status.error shouldContain "I2C"
    }

    test("should have stable LCD display default constants") {
    ...
    test("should chunk text to LCD line width") {
