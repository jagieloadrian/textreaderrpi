...
void display_text(const char *text, const char *effect, int speed_ms) {
    ...
    if (strcmp(effect, "SCROLL") == 0) {
        ...
        delay(sp * 8); s_oled.stopscroll();
    } else if (strcmp(effect, "FADE") == 0) {
        render(text);
        int sd = sp / 32 + 1;
        for (int v = 0; v <= 255; v += 8) { s_oled.ssd1306_command(SSD1306_SETCONTRAST); s_oled.ssd1306_command(v); delay(sd); }
        for (int v = 255; v >= 0; v -= 8) { s_oled.ssd1306_command(SSD1306_SETCONTRAST); s_oled.ssd1306_command(v); delay(sd); }
        s_oled.ssd1306_command(SSD1306_SETCONTRAST); s_oled.ssd1306_command(0xCF);
    }
}
